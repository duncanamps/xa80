...
type
  ...
  TXlibEnvironmentSource = (esDefault,esEnvironment,esCommandline);

  TXlibEnvironmentElement = class(TObject)
    public
      Key:    string;
      Data:   string;
      Source: TXlibEnvironmentSource;
//      function Compare(constref left, right: TXlibEnvironmentElement): integer;
      {
  ...
  TEnvComparer = specialize IComparer<TXlibEnvironmentElement>;
  ...
  TXlibEnvironment = class(specialize TSortedList<TXlibEnvironmentElement>)
    private
      EnvComparer: specialize IComparer<TXlibEnvironmentElement>;
    public
      Build:       string;
      ...
      procedure Dump;
      procedure ExpandIncludes(_src: TXlibEnvironmentSource);
      function  GetSource(_key: string): TXlibEnvironmentSource;
      function  GetValue(_key: string): string;
      function  GetValueAsInteger(_key: string): integer;
      procedure ProcessCommandLine;
      procedure ProcessCommandList(_elements: TCommandElementList; _src: TXlibEnvironmentSource);
      procedure ProcessEnvironmentVariable;
      procedure SetValue(_key: string; _data: boolean; _source: TXlibEnvironmentSource);
      procedure SetValue(_key: string; _data: integer; _source: TXlibEnvironmentSource);
      procedure SetValue(_key: string; _data: string;  _source: TXlibEnvironmentSource);
  end;

var
  EnvObject: TXlibEnvironment;  // Global environment object
...
function EnvironmentSourceAsString(_es: TXlibEnvironmentSource): string;
begin
  case _es of
    ...
    otherwise
      ErrorObj.Show(ltInternal,X3999_UNHANDLED_EXCEPTION,[Format('Internal error - Environment source %s not catered for',[GetEnumName(TypeInfo(TXlibEnvironmentSource),Ord(_es))])]);
  end;
end;
...
//------------------------------------------------------------------------------
//
//  TXlibEnvironment code
//
//------------------------------------------------------------------------------

constructor TXlibEnvironment.Create(_setdefaults: boolean);
var aVersionInfo: TVersionInfo;
    VersionInfo:  TVersion;
begin
  EnvComparer := specialize TComparer<TXlibEnvironmentElement>.Construct(@Compare);
...
destructor TXlibEnvironment.Destroy;
begin
  while Count > 0 do
    begin
      Items[Count-1].Free;
...
procedure TXlibEnvironment.AddSourceName(const _filename: string);
var s: string;
    folder: string;
    Info: TSearchRec;
...
procedure TXlibEnvironment.ApplyDefaults;
begin
...
procedure TXlibEnvironment.ExpandIncludes(_src: TXlibEnvironmentSource);
var sl: TStringList;
    i:  integer;
begin
  sl := TStringList.Create;
...
function TXlibEnvironment.GetSource(_key: string): TXlibEnvironmentSource;
var _rec: TXlibEnvironmentElement;
    _index: SizeInt;
begin
  Result := esDefault;
  // Check if key exists, if not flag as an error
  _rec := TXlibEnvironmentElement.Create;
  try
    _rec.Key  := _key;
    _rec.Data := '';
...
function TXlibEnvironment.GetValue(_key: string): string;
var _rec: TXlibEnvironmentElement;
    _index: SizeInt;
begin
  Result := '';
...
function TXlibEnvironment.GetValueAsInteger(_key: string): integer;
var s: string;
begin
  GetValueAsInteger := 0;
  try
...
procedure TXlibEnvironment.ProcessCommandLine;
...
procedure TXlibEnvironment.ProcessCommandList(_elements: TCommandElementList; _src: TXlibEnvironmentSource);
var cmd_list: TCommandList;
  ...
  procedure ProcessOption(const _name: string; const _data: string; _src: TXlibEnvironmentSource);
  begin
...
procedure TXlibEnvironment.ProcessEnvironmentVariable;
...
procedure TXlibEnvironment.SetValue(_key: string; _data: boolean; _source: TXlibEnvironmentSource);
begin
  SetValue(_key,BoolToStr(_data),_source);
end;

procedure TXlibEnvironment.SetValue(_key: string; _data: integer; _source: TXlibEnvironmentSource);
begin
  SetValue(_key,IntToStr(_data),_source);
end;

procedure TXlibEnvironment.SetValue(_key: string; _data: string; _source: TXlibEnvironmentSource);
var _rec: TXlibEnvironmentElement;
    _index: SizeInt;
begin

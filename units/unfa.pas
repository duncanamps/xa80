...
type
  ...
  TSet = class(specialize TSortedList<TSetIndex>)
    public
      procedure Add(_index: TSetIndex); reintroduce;
      function  AsString(_width: integer = -1): string;
      function  Equal(_set: TSet): boolean;
      procedure Remove(_index: TSetIndex);
      procedure Subtract(_set: TSet);
      procedure Union(_set: TSet);
  end;
...
procedure TSet.Add(_index: TSetIndex);
begin
  if IndexOf(_index) < 0 then
    inherited Add(_index);
end;

function TSet.AsString(_width: integer): string;
var idx: TSetIndex;
begin
  Result := '';
  for idx in Self do
...
function TSet.Equal(_set: TSet): boolean;
var i: integer;
begin
...
procedure TSet.Remove(_index: TSetIndex);
var _pos: integer;
begin
  _pos := IndexOf(_index);
  if _pos >= 0 then
...
procedure TSet.Subtract(_set: TSet);
var i: TSetIndex;
begin
  for i in _set do
    Remove(i);
...
procedure TSet.Union(_set: TSet);
var i: TSetIndex;
begin
  for i in _set do
    Add(i);
  ...
  begin
    if not checked[_index] then
      begin
        ...
        checked[_index] := True;
        for k := 0 to Items[_index].Epsilons.Count-1 do

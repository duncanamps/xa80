...
uses
  Classes, SysUtils, unfa, Generics.Collections;
...
type
  ...
  TDFA = class(specialize TObjectList<TDFARecord>)
    public
      function  AddTitle(_set: TSet): TSetIndex;
      ...
      function  FindTitle(_set: TSet): TSetIndex;
      procedure Optimise;
...
function TDFA.AddTitle(_set: TSet): TSetIndex;
var _dr: TDFARecord;
begin
  Result := Count;
...
begin
  ...
  for i := 0 to Count-1 do
    if i <> _b then  // skip _b as we are deleting anyway
      for c in char do
        if Items[i].Destinations[c] = _b then
          Items[i].Destinations[c] := _a;
...
begin
  ...
  // Finally, optimise the DFA ahd rename if required
  Optimise;
  Rename;
end;
...
function TDFA.FindTitle(_set: TSet): TSetIndex;
var found: boolean;
    index: TSetIndex;
begin
  index := 0;
...
begin
  ...
  try
    for c in _dictionary do
      begin
        _set.Clear;
        for j := 0 to Items[_index].Title.Count-1 do
          begin
...
begin
  ...
  //    DWORD      magic_marker3    contains 1A 0A 41 46   ("FA" + <lf> + <eol>)
